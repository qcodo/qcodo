...
<?php
	// Filename Cleanup
	if (($strScript == 'header.inc.php') || ($strScript == 'footer.inc.php'))
		$strFilename = __INCLUDES__ . '/examples/' . $strScript;
	else if ($strScript == 'examples.css')
		$strFilename = __DOCROOT__ . __SUBDIRECTORY__ . __CSS_ASSETS__ . '/examples/' . $strScript;
	else if (($strScript == 'mysql_innodb.sql') || ($strScript == 'sql_server.sql')) {
		$strFilename = __INCLUDES__ . '/examples/database/' . $strScript;
