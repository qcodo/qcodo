<?php
	...
	class QMySqliDatabaseField extends QDatabaseFieldBase {
		public function __construct($mixFieldData, $objDb = null) {
			...
			// MySQL/MariaDB reports a native UUID column as a plain string, so the column definition
			// from DESCRIBE (e.g. "uuid") is the only way to identify one
			$this->blnUuid = (strtolower(trim((string) $this->strOriginalType)) == 'uuid');

			// A column is an "Identity" column if its value is generated for it on INSERT.  This is the case for
			// any AUTO_INCREMENT column, and also for any UUID PK column (whose value gets generated by Qcodo)
			$this->blnIdentity = (($mixFieldData->flags & MYSQLI_AUTO_INCREMENT_FLAG) ||
				($this->blnPrimaryKey && $this->blnUuid)) ? true : false;
